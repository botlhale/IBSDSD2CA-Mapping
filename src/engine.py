...
class MappingEngine:
    ...
    def _evaluate_formula(self, formula: str, gq_data: Dict[int, float]) -> float:
        ...
        try:
            ...
            for code_str in codes:
                code = int(code_str)
                value = gq_data.get(code, 0.0)
                # Replace the code with its value, being careful about partial matches
                # Use word boundaries to avoid replacing parts of larger numbers
    ...
    def generate_report(self, gq_data: Dict[int, float], report_type: str) -> List[DSDDataPoint]:
        """
        ...
        if report_type not in ['lbsr', 'lbsn']:
            raise ValueError(f"Invalid report type: {report_type}. Must be 'lbsr' or 'lbsn'")
        ...
        for rule in mapping_rules:
            try:
                ...
                raise ValueError(f"Error processing rule for {rule.get('dsd_code', 'unknown')}: {str(e)}")
    ...
    def validate_rules(self, gq_codes: List[int]) -> List[str]:
        ...
        for report_type in ['lbsr', 'lbsn']:
            ...
            for rule in mapping_rules:
                ...
                code_pattern = r'\b\d+\b'
                formula_codes = [int(code) for code in re.findall(code_pattern, formula)]
                
                # Check if all codes exist in the available GQ codes
                missing_codes = [code for code in formula_codes if code not in gq_code_set]
